...
#include "pol_falcon_1024_signTime.h"
#include <stdbool.h>
#include <stdio.h>
#include "stdlib.h"
#include <string.h>
...
// ===================================================================================================================================================
    ...
    PoLNode::InitializeKey(void) {
        ...
        uint8_t public_key[OQS_SIG_falcon_1024_length_public_key];
        uint8_t secret_key[OQS_SIG_falcon_1024_length_secret_key];

        cout << "keyGen Start" << endl;
        clock_t start = clock();
        for (int i = 0; i < network.numTx; i++) {
            OQS_SIG_falcon_1024_keypair(public_key, secret_key);
        }
        clock_t end = clock();
        ...
        OQS_STATUS rc = OQS_SIG_falcon_1024_keypair(public_key, secret_key);
        m_pk = network.Array2Bytes(public_key, OQS_SIG_falcon_1024_length_public_key);
        m_sk = network.Array2Bytes(secret_key, OQS_SIG_falcon_1024_length_secret_key);
    }
    ...
    PoLNode::SignTx(vector<uint8_t> txItem, vector<uint8_t> sk) {
        ...
        clock_t start = clock();
        OQS_SIG_falcon_1024_sign(signature, &signature_len, message, message_len, secret_key);
        clock_t end = clock();
    ...
    PoLNode::VerifyTx(vector<uint8_t> txItem, vector<uint8_t> sign, vector<uint8_t> pk) {
        ...
        clock_t start = clock();
        for (int i = 0; i < network.numTx; i++) {
            OQS_SIG_falcon_1024_verify(message, message_len, signature, signature_len, public_key);
        }
        clock_t end = clock();
        ...
        rc = OQS_SIG_falcon_1024_verify(message, message_len, signature, signature_len, public_key);
