...
namespace ns3 
{
    ...
    class Network 
    {
        public:
            ...
            vector<int>                 stakingVolume;                                              // 각 노드의 스테이킹값을 저장해둔 벡터
    ...
    class Block 
    {
        public:
            // ===== 생성자 & 소멸자 ===== //
            Block(void);                                                                   
            Block(int blockId, vector<uint8_t> prevHash, int coinAge, vector <Tx> tx);     
            virtual ~Block(void);                                                          
            ...
            class BlockHeader                                                                       // 블록 헤더
            {
                public:
                    ...
                    // ===== 변수 ===== //
                    int                  blockId;                                                   // 블록 라운드 (Id)
                    int                  coinAge;                                                   // 코인 나이
                    vector <uint8_t>     prevHash;                                                  // 이전 블록 해시
            ...
            vector<uint8_t> SerializeCoinAge(void);                                                 // Serialize coinage
    ...
    class PoS : public Application 
    {
      public:
        ...
        void                                    HandleRead (Ptr<Socket> socket);                                                 // 전송 받은 메시지 처리하는 함수
        std::string                             GetPacketContent(Ptr<Packet> packet, Address from);                              // 패킷 내의 메시지를 string으로 파싱

        void                                    SendData(vector<uint8_t> vect_data);                                             // 모든 인접 노드에게 블록 브로드캐스트
        void                                    CoinageCalc(void);                                                               // 할당받은 코인 중 일부를 스테이킹
        bool                                    ValidationBlock(Block receive_block);                                            // 전송 받은 블록을 검증
        Block                                   generateBlock(unsigned int age);                                                 // 블록을 생성
        void                                    CoinAllocation(void);                                                            // 노드에게 임의로 코인 할당
        Tx                                      GenerateTx(void);                                                                // Generate Transaction            
        vector<uint8_t>                         SignTx(vector<uint8_t> txItem, vector<uint8_t> sk);                              // Sign Transaction
        vector<uint8_t>                         GetBlockHash(Block block);                                                       // Calculate hash of block
        ...
        Ptr<Socket>                             m_socket;                       // monitor socket
        std::map<Ipv4Address, Ptr<Socket>>      m_peersSockets;                 // 인접 노드의 소켓 목록
        std::map<Address, std::string>          m_bufferedData;                 // 이전 handleRead 이벤트에서 버퍼링된 데이터를 보유하는 맵
        Address                                 m_local;                        // 지금 노드의 주소
        std::vector<Ipv4Address>                m_peersAddresses;               // 인접 노드 리스트
        map<Ipv4Address, uint32_t>              m_peersIndex;                   // 다른 노드들의 index

        int                                     numNodes;                       // 총 노드 수
        int                                     round;                          // 합의 라운드
        Blockchain                              m_Blockchain;                   // 노드별 분산 원장
        queue<Tx>                               m_memPool;                      // 메모리 풀


        int                                     intCoinAge;                     // 코인 나이 (integer)
        double                                  coin_volume;                    // 노드의 코인 보유량
        int                                     staking_Volume;                 // 스테이킹한 코인의 양
        int                                     staking_day;                    // 스테이킹한 시간 (라운드))
        
        int                                     m_stakingIdx;                   // 노드의 index (자기자신) 
