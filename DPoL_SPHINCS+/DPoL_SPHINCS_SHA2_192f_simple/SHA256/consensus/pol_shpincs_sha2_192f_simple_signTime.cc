...
#include "pol_shpincs_sha2_192f_simple_signTime.h"
#include <stdbool.h>
#include <stdio.h>
#include "stdlib.h"
#include <string.h>
...
// ===================================================================================================================================================
    ...
    Network::Network(void) {
        ...
        prevHashSize = 64;                          // 256  bits (SHA-256)
...
// ===================================================================================================================================================
    ...
    TEE::LuckInProof(vector<uint8_t> proof) {
        ...
        string hex;
        for (int i = 0; i < 8; i++) { hex += data.at(64 + i); }
    ...
    TEE::NonceInProof(vector<uint8_t> proof) {
        ...
        string hex;
        for(int i = 0; i < 64; i++) { hex += data.at(i); }
...
// ===================================================================================================================================================
    ...
    Block::GetBlockHash() {
        ...
        for (int i = 0; i < 64; i++) { blockHashValue.push_back(temp[i]); }
    ...
    Block::DeserializeBlock(string packet) {
        ...
        do {
            ...
            switch(offset) {
                ...
                // 'k'ey
                case 'k':
                    this->header.txs[idx].pk.push_back(packet[i]);
                    continue;

                // 's'ign
                case 's':
                    this->header.txs[idx].sign.push_back(packet[i]);
                    continue;
    ...
    Block::BlockHeader::GetHeaderHash() {
        ...
        for (size_t i = 0; i < this->txs.size(); i++) {
            ...
            for (size_t k = 0; k < this->txs[i].sign.size(); k++) {
                str += this->txs[i].sign[k];
            }
            for (size_t l = 0; l < this->txs[i].pk.size(); l++) {
                str += this->txs[i].pk[l];
            }
        }
        ...
        for (int i = 0; i < 64; i++) { headerHashValue.push_back(temp[i]); }
...
// ===================================================================================================================================================
        ...
        while(packet = socket->RecvFrom(from)) {
            ...
            if (InetSocketAddress::IsMatchingType(from)) {
                ...
                if (msg[0] == 'x') {
                    ...
                    Blockchain chain;
                    chain.DeserializeChain(msg);

                    Valid(chain);
                    VerifyTxs(chain);


                    // cout << "print new chain" << endl;
                    // chain.PrintChain();

                    float newChainLuck = Luck(chain);
                    // cout << "new chain's luck: " << newChainLuck << endl;

                    float originalChainLuck = Luck(m_chain);
                    // cout << "original chain's luck: " << originalChainLuck << endl;

                    // ===== Comparison Luck Value ===== //
                    if (Luck(chain) > Luck(m_chain)) {
                        // cout << "!!! Node[" << GetNode()->GetId() << "]'s chain has changed !!!" << endl;
                        m_chain = chain;
    ...
    PoLNode::InitializeKey(void) {
        ...
        uint8_t public_key[OQS_SIG_sphincs_sha2_192f_simple_length_public_key];
        uint8_t secret_key[OQS_SIG_sphincs_sha2_192f_simple_length_secret_key];

        cout << "keyGen Start" << endl;
        clock_t start = clock();
        for (int i = 0; i < network.numTx; i++) {
            OQS_SIG_sphincs_sha2_192f_simple_keypair(public_key, secret_key);
        }
        clock_t end = clock();
        network.keyGenTime = double(end - start) / network.numTx / CLOCKS_PER_SEC;
        cout << "keyGenTime: " << network.keyGenTime << endl;
        sleep(7);

        OQS_STATUS rc = OQS_SIG_sphincs_sha2_192f_simple_keypair(public_key, secret_key);
        m_pk = network.Array2Bytes(public_key, OQS_SIG_sphincs_sha2_192f_simple_length_public_key);
        m_sk = network.Array2Bytes(secret_key, OQS_SIG_sphincs_sha2_192f_simple_length_secret_key);
    }
    ...
    PoLNode::InitializeMempool(void) {
        ...
        cout << "SignTime: " << network.signTime / (GetNode()->GetId() + 1) << endl;
    ...
    PoLNode::InitializeChain(void) {
        ...
        genesisBlock.body.proof = network.String2Bytes("iFiY4cAcuEq6c5Bx2QKnja3XGadaghm9zRcmtgTrgMQU6vRCr56xhUY5xA5gsZ6ZQ96THB5LizwKNtWbVNsYY4sx1Pw2XUTzBh");
    ...
    vector<uint8_t>
    PoLNode::SignTx(vector<uint8_t> txItem, vector<uint8_t> sk) {
        uint8_t signature[OQS_SIG_sphincs_sha2_192f_simple_length_signature] = {};
        ...
        clock_t start = clock();
        OQS_SIG_sphincs_sha2_192f_simple_sign(signature, &signature_len, message, message_len, secret_key);
        clock_t end = clock();
        network.signTime += double(end - start) / network.numTx / CLOCKS_PER_SEC;
        // signature[signature_len] = '\0';
        vector<uint8_t> sig = network.Array2Bytes(signature, OQS_SIG_sphincs_sha2_192f_simple_length_signature);        
        return sig;
    }
    ...
    bool
    PoLNode::VerifyTx(vector<uint8_t> txItem, vector<uint8_t> sign, vector<uint8_t> pk) {
        size_t message_len = txItem.size()/2;
        uint8_t message[message_len];
        network.Bytes2Array(txItem, message);
        // message[message_len] = '\0';
        ...
        clock_t start = clock();
        for (int i = 0; i < network.numTx; i++) {
            OQS_SIG_sphincs_sha2_192f_simple_verify(message, message_len, signature, signature_len, public_key);
        }
        clock_t end = clock();
        network.verifyTime = double(end - start) / network.numTx / CLOCKS_PER_SEC;
        cout << "verifyTime: " << network.verifyTime  << endl;
        sleep(7);

        rc = OQS_SIG_sphincs_sha2_192f_simple_verify(message, message_len, signature, signature_len, public_key);
